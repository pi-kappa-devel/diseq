...
setMethod("initialize", "simulated_model", function(
  .Object,
  nobs, tobs,
...
setMethod("get_demanded_quantities", signature(object = "simulated_model"), function(object, prices) {
  as.vector(
...
setMethod("get_supplied_quantities", signature(object = "simulated_model"), function(object, prices) {
  as.vector(
...
setMethod("simulate_column", signature(object = "simulated_model"), function(
...
setGeneric("simulate_quantities_and_prices", function(
  object, demanded_quantities = NA, supplied_quantities = NA, prices = NA, starting_prices = NA
) {
  standardGeneric("simulate_quantities_and_prices")
})

setMethod("simulate_quantities_and_prices", signature(object = "simulated_model"), function(
...
setMethod("simulate_quantities_and_prices", signature(object = "simulated_equilibrium_model"), function(
  object, demanded_quantities, supplied_quantities, prices, starting_prices
) {
...
setMethod("simulate_quantities_and_prices", signature(object = "simulated_basic_model"), function(
  object, demanded_quantities, supplied_quantities, prices, starting_prices
) {
  scale <- (object@alpha_d - object@alpha_s)
  x_d <- get_demand_controls(object)
...
setMethod("simulate_quantities_and_prices", signature(object = "simulated_directional_model"), function(
  object, demanded_quantities, supplied_quantities, prices, starting_prices
) {
  ...
  for (i in 1:object@nobs) {
    ...
    for (t in 1:object@tobs) {
      ...
      repeat {
        ...
        object@simulation_tbl[i_offset + t, demand_columns] <- object@control_generator(length(demand_columns))
        object@simulation_tbl[i_offset + t, supply_columns] <- object@control_generator(length(supply_columns))
        object@simulation_tbl[i_offset + t, c("u_d", "u_s")] <- MASS::mvrnorm(n = 1, object@mu, object@sigma)
        r_d[i_offset + t] <- (
...
setMethod("simulate_quantities_and_prices", signature(object = "simulated_deterministic_adjustment_model"), function(
  object, demanded_quantities, supplied_quantities, prices, starting_prices
) {
  r_d <- get_demanded_quantities(object, 0)
  r_s <- get_supplied_quantities(object, 0)
...
setMethod("initialize", "simulated_deterministic_adjustment_model", function(
  .Object,
  nobs, tobs,
  alpha_d, beta_d0, beta_d, eta_d,
  alpha_s, beta_s0, beta_s, eta_s,
...
setMethod("initialize", "simulated_stochastic_adjustment_model", function(
  .Object,
  nobs, tobs,
  alpha_d, beta_d0, beta_d, eta_d,
  alpha_s, beta_s0, beta_s, eta_s,
...
setMethod("get_price_controls", signature(object = "simulated_stochastic_adjustment_model"), function(object) {
  as.matrix(object@simulation_tbl[, grep("Xp", names(object@simulation_tbl))])
})

setMethod("simulate_controls", signature(object = "simulated_stochastic_adjustment_model"), function(object) {
...
setMethod("simulate_shocks", signature(object = "simulated_stochastic_adjustment_model"), function(object) {
  cov_ds <- object@sigma_d * object@sigma_s * object@rho_ds
  ...
  object@mu <- c(0, 0, 0)
  object@sigma <- matrix(
    c(object@sigma_d ** 2, cov_ds, cov_dp, cov_ds, object@sigma_s ** 2, cov_sp, cov_dp, cov_sp, object@sigma_p ** 2),
    3, 3
  )
...
#' Simulate model data.
#'
#' Returns a data tibble with simulated data from a generating process that matches the passed model string. The
#' simulated observations of the controls are drawn from a uniform distribution on the interval [2, 5].
...
#' @export
setGeneric("simulate_model_data", function(
...
#' @rdname simulate_model_data
setMethod("simulate_model_data", signature(), function(
  model_string, nobs, tobs,
  alpha_d, beta_d0, beta_d, eta_d,
  alpha_s, beta_s0, beta_s, eta_s,
  gamma, beta_p0, beta_p,
  ...
  if (model_string %in% c("eq_2sls", "eq_fiml")) {
    ...
    stopifnot(abs(sim_mdl@gamma * sim_mdl@simulation_tbl$DP - sim_mdl@simulation_tbl$XD) < sqrt(.Machine$double.eps))
    xd_share <- sum(sim_mdl@simulation_tbl$XD > 0) / nrow(sim_mdl@simulation_tbl)
    stopifnot(xd_share < 0.9 && xd_share > 0.1)
  }
...
setMethod("simulate_model", signature(), function(
  ...
  demand_specification <- paste(
    ...
    paste("X", seq_along(eta_d), sep = "", collapse = " + "), sep = " + "
  )
  supply_specification <- paste(
    paste("Xs", seq_along(beta_s), sep = "", collapse = " + "),
    paste("X", seq_along(eta_s), sep = "", collapse = " + "), sep = " + "
  )
