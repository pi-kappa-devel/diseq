...
setMethod("initialize", "system_deterministic_adjustment", function(
...
setMethod("get_likelihood_variables", signature(object = "system_deterministic_adjustment"), function(object) {
  ...
  likelihood_variables <- c(
    likelihood_variables[1:pos], get_price_differences_variable(object), likelihood_variables[(pos + 1):len]
  )

  likelihood_variables
})

setMethod("set_parameters", signature(object = "system_deterministic_adjustment"), function(object, parameters) {
...
setMethod("calculate_system_moments", signature(object = "system_deterministic_adjustment"), function(object) {
    object@mu_P <- (
    ...
    object@demand@z_PQ <- object@demand@rho1_PQ * object@h_P - object@demand@rho2_PQ * object@demand@h_Q
    object@supply@z_PQ <- object@supply@rho1_PQ * object@h_P - object@supply@rho2_PQ * object@supply@h_Q

    object@demand@z_QP <- object@demand@rho1_PQ * object@demand@h_Q - object@demand@rho2_PQ * object@h_P
    object@supply@z_QP <- object@supply@rho1_PQ * object@supply@h_Q - object@supply@rho2_PQ * object@h_P
...
setMethod("calculate_system_loglikelihood", signature(object = "system_deterministic_adjustment"), function(object) {
  lld <- - log(2 * pi) - log(object@sigma_P * object@demand@sigma_Q / object@demand@rho1_PQ) - (
    object@demand@rho1_PQ ** 2 * (
      object@h_P ** 2 - 2 * object@h_P * object@demand@h_Q * object@demand@rho_PQ + object@demand@h_Q ** 2
    )
  ) / 2

  lls <- - log(2 * pi) - log(object@sigma_P * object@supply@sigma_Q / object@supply@rho1_PQ) - (
    object@supply@rho1_PQ ** 2 * (
      object@h_P ** 2 - 2 * object@h_P * object@supply@h_Q * object@supply@rho_PQ + object@supply@h_Q ** 2
    )
  ) / 2
...
setMethod("calculate_system_gradient", signature(object = "system_deterministic_adjustment"), function(object) {
  pd_alpha_d <- (
  ...
  object@gradient <- cbind(
    ...
    sum(pd_gamma * object@demand@separation_subset) + sum(ps_gamma * object@supply@separation_subset),
    sum(pd_sigma_d * object@demand@separation_subset) + sum(ps_sigma_d * object@supply@separation_subset),
    sum(pd_sigma_s * object@demand@separation_subset) + sum(ps_sigma_s * object@supply@separation_subset)
  )
  if (object@correlated_shocks) {
    object@gradient <- cbind(
      object@gradient,
      sum(pd_rho * object@demand@separation_subset) + sum(ps_rho * object@supply@separation_subset)
    )
  }
