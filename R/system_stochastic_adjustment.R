...
setMethod("initialize", "system_stochastic_adjustment", function(
  .Object, quantity, price, demand_specification, supply_specification, price_specification, data, correlated_shocks
) {
...
setMethod("get_likelihood_variables", signature(object = "system_stochastic_adjustment"), function(object) {
  ...
  if (object@correlated_shocks) {
    ...
    likelihood_variables <- c(likelihood_variables, get_prefixed_variance_variable(object@price_equation))
  }

  likelihood_variables
})

setMethod("set_parameters", signature(object = "system_stochastic_adjustment"), function(object, parameters) {
...
setMethod("calculate_system_moments", signature(object = "system_stochastic_adjustment"), function(object) {
  object@mu_P <-
    (
      object@lagged_price_vector * object@gamma + object@demand@control_matrix %*% object@demand@beta +
  ...
  object@var_D <-
    (
      ...
      + 2 * object@demand@alpha * object@gamma * object@rho_dp * object@demand@sigma * object@price_equation@sigma
  ...
  object@var_S <-
    (
      ...
      + 2 * object@supply@alpha * object@gamma * object@rho_sp * object@price_equation@sigma * object@supply@sigma
  ...
  object@cov_DS <-
    (
      object@demand@alpha * object@supply@alpha * object@delta * object@var_P + object@demand@alpha * object@gamma *
        object@rho_sp * object@price_equation@sigma * object@supply@sigma - object@demand@alpha *
        object@supply@sigma ** 2 + object@supply@alpha * object@gamma * object@rho_dp * object@demand@sigma *
        object@price_equation@sigma + object@supply@alpha * object@demand@sigma ** 2 + object@gamma * object@rho_ds *
        object@demand@sigma * object@supply@sigma
    ) / object@delta

  object@cov_DP <-
    (
      object@demand@alpha * object@delta * object@var_P + object@gamma * object@rho_dp * object@demand@sigma *
  ...
  object@cov_SP <-
    (
      object@supply@alpha * object@delta * object@var_P + object@gamma * object@rho_sp * object@price_equation@sigma *
  ...
  object@omega_D <- (
    -object@h_D * object@zeta_DS / sqrt(object@zeta_SS) - object@h_P * object@zeta_SP / sqrt(object@zeta_SS) +
      object@h_S * sqrt(object@zeta_SS)
  )
  object@omega_S <- (
    object@h_D * sqrt(object@zeta_DD) - object@h_P * object@zeta_DP / sqrt(object@zeta_DD) - object@h_S *
      object@zeta_DS / sqrt(object@zeta_DD)
  )
  ...
  object@g_D <- dnorm(object@omega_D / object@zeta) / pnorm(object@omega_D / object@zeta, lower.tail = FALSE)
  object@g_S <- dnorm(object@omega_S / object@zeta) / pnorm(object@omega_S / object@zeta, lower.tail = FALSE)
...
setMethod("calculate_system_loglikelihood", signature(object = "system_stochastic_adjustment"), function(object) {
  object@log_likelihood <- log(object@L_D + object@L_S)
  object
})

setMethod("calculate_system_gradient", signature(object = "system_stochastic_adjustment"), function(object) {
  ...
  object@gradient <- cbind(
    p_dprice, t(p_dcontrols), p_sprice, t(p_scontrols), p_pdiff, t(p_pcontrols), p_vard, p_vars, p_varp
  )
  if (object@correlated_shocks) {
